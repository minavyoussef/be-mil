...
DATE_BEGIN = datetime.strptime('1990-01-01', '%Y-%m-%d')

TRAINING_DATE_TO = datetime.strptime('2019-07-01', '%Y-%m-%d')

VALIDATION_DATE_FROM = datetime.strptime('1990-01-01', '%Y-%m-%d')
...
class SingleInstrumentPredictorRNN(nn.Module):
    ...
    def __init__(self):
        ...
        self._conv1d = nn.Conv1d(1, 1, 3, padding=)
    ...
    def init_hidden(self, batch_size):
        ...
        hidden = (weight.new(HyperParams.Model.RNN.layers_size,
                  ...
                  weight.new(HyperParams.Model.RNN.layers_size,
                             batch_size, HyperParams.Model.RNN.hidden_size).zero_().double().to(device))
...
model.load_state_dict(torch.load(f'./{INSTRUMENT}_{HyperParams.training.epochs}epochs_miniloss_model_state.pt'))
model.eval()
...
for test_example in testing_dataset:
    ...
    preidcted_list.append(output[0][-1])
    expected_list.append(batch_target[0][-1])
